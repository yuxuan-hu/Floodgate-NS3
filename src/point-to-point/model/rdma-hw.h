...
public:
	...
	typedef Callback<void, Ptr<RdmaRxQueuePair> > RcvFinishCallback;
	typedef Callback<void, Ptr<RdmaOperation> > MsgCompleteCallback;
	 ...
	 * for qp mode
	 */
	Ptr<RdmaQueuePair> GetQp(uint32_t qpid); // wqy, get the qp by qpid
	void MsgComplete(Ptr<RdmaQueuePair> qp, Ptr<RdmaOperation> msg);
	void ResetQP(Ptr<RdmaQueuePair> qp);

	void SetNode(Ptr<Node> node);
	void Setup(QpCompleteCallback cb, RcvFinishCallback rcvcb, MsgCompleteCallback mcb); // setup shared data and callbacks with the QbbNetDevice
	...
	Ptr<RdmaRxQueuePair> GetRxQp(uint32_t sip, uint32_t dip, uint16_t sport, uint16_t dport, uint16_t pg, uint32_t flow_id, bool create); // get a rxQp
	...
	void RcvFinish(Ptr<RdmaRxQueuePair> qp);
	...
	void HandleDisOrderedAckDctcp(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch);
};
